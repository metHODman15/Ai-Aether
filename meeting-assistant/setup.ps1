...
if (Test-Path ".env") {
    ...
    if ($whisperBackendValue -eq "local") {
        ...
        $openAILabel = "OpenAI API key (required for CRM entity extraction; NOT used for transcription in local mode)"
    } else {
        Write-Info "  Selected: openai backend — audio sent to OpenAI Whisper API."
        $openAILabel = "OpenAI API key (required for Whisper transcription AND CRM entity extraction)"
    }

    Write-Host ""
    Write-Host "── API Keys ──────────────────────────────────────────" -ForegroundColor Cyan
    $lineOpenAI    = Prompt-Required "OPENAI_API_KEY"    $openAILabel
