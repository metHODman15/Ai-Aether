...
if [ -f ".env" ]; then
    ...
    LINE_ANTHROPIC=$(prompt_required "ANTHROPIC_API_KEY" "Anthropic API key (Claude — topic detection and entity extraction)")
    if $NEED_OPENAI; then
        LINE_OPENAI=$(prompt_required "OPENAI_API_KEY" "OpenAI API key (Whisper transcription)")
    else
        info "  OPENAI_API_KEY not required in local mode (entity extraction uses Anthropic)."
        LINE_OPENAI=$(prompt_optional "OPENAI_API_KEY" "OpenAI API key (optional — only needed if you switch to openai backend)" "")
    fi
