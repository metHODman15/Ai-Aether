...
if [ -f ".env" ]; then
    ...
    # Generate a random 32-byte hex ENCRYPTION_KEY if the user leaves it blank.
    prompt_encryption_key() {
        read -rp "  Encryption key (leave blank to auto-generate): " value
        if [ -z "$value" ]; then
            # Use Python (already confirmed available) to generate a secure key.
    ...
    echo -e "${BOLD}── Transcription backend ─────────────────────────────${RESET}"
    echo "  Two Whisper backends are available:"
    echo "    openai  — Audio sent to the OpenAI Whisper API (default, no extra install)"
    echo "    local   — Transcription runs entirely on-device with faster-whisper (free, private)"
    WHISPER_BACKEND_VALUE=""
    while [ "$WHISPER_BACKEND_VALUE" != "openai" ] && [ "$WHISPER_BACKEND_VALUE" != "local" ]; do
        read -rp "  Choose backend [openai]: " WHISPER_BACKEND_VALUE
        WHISPER_BACKEND_VALUE="${WHISPER_BACKEND_VALUE:-openai}"
        if [ "$WHISPER_BACKEND_VALUE" != "openai" ] && [ "$WHISPER_BACKEND_VALUE" != "local" ]; then
            warn "  Enter 'openai' or 'local'."
        fi
    done
    LINE_WHISPER_BACKEND="WHISPER_BACKEND=${WHISPER_BACKEND_VALUE}"

    if [ "$WHISPER_BACKEND_VALUE" = "local" ]; then
        info "  Selected: local backend — audio never leaves your machine."
        info "  You'll need to run:  pip install faster-whisper"
        info "  Model weights are downloaded automatically on first start."
        OPENAI_LABEL="OpenAI API key (required for CRM entity extraction; NOT used for transcription in local mode)"
    else
        info "  Selected: openai backend — audio sent to OpenAI Whisper API."
        OPENAI_LABEL="OpenAI API key (required for Whisper transcription AND CRM entity extraction)"
    fi

    echo
    echo -e "${BOLD}── API Keys ──────────────────────────────────────────${RESET}"
    LINE_OPENAI=$(prompt_required  "OPENAI_API_KEY"    "$OPENAI_LABEL")
...
# OpenAI API key.
# Always required: CRM entity extraction always uses the OpenAI chat API.
# Also required for Whisper transcription when WHISPER_BACKEND=openai.
${LINE_OPENAI}
...
echo "On first launch, click 'Connect to Salesforce' to authorize via OAuth."
echo
