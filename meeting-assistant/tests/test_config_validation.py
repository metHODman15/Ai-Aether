...
def test_skip_startup_validation_short_circuits():
    ...
    assert result == {"anthropic": "skipped", "salesforce": "oauth_flow"}


def test_salesforce_always_returns_oauth_flow_status(monkeypatch):
    """Salesforce is validated through the OAuth UI flow, not at startup.
    validate_credentials must always return 'oauth_flow' for salesforce,
    ...
    assert result["salesforce"] == "oauth_flow"
